...
public class SearchComponent {
    ...
    public List<Flight> search(SearchQuery query) {
        List<Flight> flights =
                flightRepository.findByOriginAndDestinationAndFlightDate(query.getOrigin(), query.getDestination(), LocalDate.parse(query.getFlightDate()));
        return flights.stream().filter(flight -> flight.getInventory().getCount() > 0).collect(Collectors.toList());
    }
    ...
    public void updateInventory(String flightNumber, LocalDate flightDate, int inventory) {
        ...
        Flight flight = flightRepository.findByFlightNumberAndFlightDate(flightNumber, flightDate).orElseThrow(() -> new FlightNotFoundException());
